...
const App= ( props ) =>{
    ...
    return <div className={`wp-block-themehunk-product-slider`} >
 
    
    
    <ProductSlider  styles={props.style} orderBy = {props.orderBy} productCount={props.totalCount} totalproduct={props.totalproduct}  columns={props.columns}  key={props.columns} />
     </div>;
...
domReady( function () {
const getConteiners = document.querySelectorAll('.ubp-slider');
console.log('getConteiners',getConteiners);
getConteiners.forEach(item=>{
    const containerAttr = item.getAttribute('ubp-slider');
